...
void read_tau_solar (const char *reducedLkpPath, int nLev, double *pLev, double *T, double *H2O_VMR,
  ...
  int retval;
  int ncid, xsec_nbooksid, xsec_npagesid, xsec_nrowsid, xsec_ncolsid, vmrs_ref_nrowsid, vmrs_ref_ncolsid, t_pert_nelemid, num_of_nodesid, nls_pert_nelemid, nls_pertid, xsecid, vmrs_refid, t_refid, p_gridid, t_pertid, ChosenWvlsid, ChosenWeightsid, solar_irradiance_id;
  ...
  */
  ...
  double solar_irradiance_array[num_of_nodes];
  ...
  /* Get the varid of the data variable, based on its name. */
    ...
    if ((retval = nc_get_var_double(ncid, ChosenWeightsid, &weights_array[0])))
       ERR(retval);
    if ((retval = nc_inq_varid(ncid, "solar_irradiance", &solar_irradiance_id)))
       ERR(retval);
    if ((retval = nc_get_var_double(ncid, solar_irradiance_id, &solar_irradiance_array[0])))
       ERR(retval);
  
  
  *wvl    = (double *)  calloc (*nWvl, sizeof(double));
  *weight = (double *)  calloc (*nWvl, sizeof(double));
  *Esolar = (double *)  calloc (*nWvl, sizeof(double));
  *tau    = (double **) calloc (*nWvl, sizeof(double *));
  ...
  for(int iwvl=0; iwvl<num_of_nodes; iwvl++){
    ...
    (*Esolar)[iwvl] = solar_irradiance_array[iwvl];
  } 
